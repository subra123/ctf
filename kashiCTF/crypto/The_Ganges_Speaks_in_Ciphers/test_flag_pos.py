...
for flag_idx in range(5):
    print(f"--- Assuming p{flag_idx} starts with {crib.decode()} ---")
    key_part = xor(c_bytes[flag_idx][:len(crib)], crib)
    for i in range(5):
        p_part = xor(c_bytes[i][:len(crib)], key_part)
        print(f"p{i}: {p_part}")
