...
for i in range(5):
    res = xor(c_bytes[i][:len(crib)], crib)
    print(f"c{i} ^ '{crib.decode()}': {res.hex()}")
    # Check for ISCII Sanskrit
    # ISCII: dc ed e6 ef eb f0 f8 ed fe
    # My c4 ^ crib was 'e2 a8 80 87 9f ...'? No.
    # Let's re-run the previous script to be sure.
