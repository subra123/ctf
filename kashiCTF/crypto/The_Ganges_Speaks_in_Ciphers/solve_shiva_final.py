...
shiva = [0xA4, 0xA2, 0xD2, 0xB1, 0xA7, 0xAB, 0xB3, 0xA9, 0xAF, 0xB2, 0xAB, 0xAD, 0xB5, 0xEF, 0xDF, 0xDC, 0xE2, 0xB8, 0xEB, 0xB1, 0xE0, 0xE1, 0xE4, 0xE3, 0xDC, 0xE1, 0xC8, 0xCD, 0xE0, 0xC9, 0xCE, 0xCF, 0xE7, 0xD1, 0xDB, 0xDD, 0xDC, 0xDE, 0xE6, 0xB4, 0xD5, 0xB6, 0xCE, 0xCF, 0xB7, 0xD2, 0xB3, 0xDC, 0xB3, 0xD4, 0xDF, 0xE6, 0xE7, 0xED, 0xE2, 0xEF, 0xEB]
shiva_bytes = bytes(shiva)

for pos in range(len(c_bytes[2]) - 10):
    for i in range(len(shiva_bytes) - 10):
        length = min(len(c_bytes[2]) - pos, len(shiva_bytes) - i)
        if length < 10: continue
        
        key_part = xor(c_bytes[2][pos:pos+length], shiva_bytes[i:i+length])
        
        # Check if p4 starts with kashiCTF{
        p4 = xor(c_bytes[4][pos:pos+length], key_part)
        if b"kashi" in p4.lower():
            print(f"Found match! p2 pos {pos} matches Shiva index {i}")
            print(f"p4: {p4}")
