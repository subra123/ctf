...
for i in range(5):
    for pos in range(len(c_bytes[i]) - len(crib) + 1):
        # Assume p_i[pos:pos+len(crib)] == crib
        key_part = xor(c_bytes[i][pos:pos+len(crib)], crib)
        
        # Count how many other messages are printable at this pos
        printable_count = 0
        others = []
        for j in range(5):
            if i == j: continue
        ...
        if printable_count >= 3:
            print(f"p{i} at pos {pos} is '{crib.decode()}':")
            for j, p_part in enumerate(others):
                print(f"  Other: {p_part}")
