...
for i in range(5):
    for j in range(i + 1, 5):
        diff = xor(c_bytes[i], c_bytes[j])
        try:
            # Try to decode as UTF-8
            decoded = diff.decode('utf-8')
            if any('\u0900' <= char <= '\u097F' for char in decoded):
                print(f"c{i} ^ c{j} contains Sanskrit!")
                print(decoded)
        except:
            # Maybe it's not full UTF-8
            pass

        # Try to decode as ISCII
        # ISCII is not a standard Python encoding, but we can check for bytes > 0xA0
        # Wait, if p1 ^ p2 is Sanskrit in ISCII, it would be b1 ^ b2
        # where b1, b2 > 0xA0. So b1 ^ b2 < 0x80.
        # My xor results already showed many bytes < 0x80.
