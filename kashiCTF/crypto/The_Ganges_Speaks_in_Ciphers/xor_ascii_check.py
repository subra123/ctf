def xor_ascii(s1, s2):
    return bytes(a ^ b for a, b in zip(s1.encode(), s2.encode()))
...
for i in range(len(c)):
    for j in range(i + 1, len(c)):
        res = xor_ascii(c[i], c[j])
        print(f"XOR c{i} ^ c{j} (ASCII): {res.hex()}")
        try:
            # Check for Sanskrit (Devanagari) in UTF-8
            # Devanagari is e0 a4 .. e0 a5 ..
            if b'\xe0\xa4' in res or b'\xe0\xa5' in res:
                print(f"Potential Sanskrit in c{i} ^ c{j}")
            print(f"Decoded: {res.decode('utf-8', errors='ignore')}")
        except:
            pass
