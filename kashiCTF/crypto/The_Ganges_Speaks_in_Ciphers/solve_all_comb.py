...
for i in range(32):
    ...
    for j in range(5):
        if (i >> j) & 1:
            current_c.append(c_bytes[j].swapcase())
        else:
            current_c.append(c_bytes[j])
    
    # Assume p4 starts with crib
    key_part = xor(current_c[4][:len(crib)], crib)
    
    # Check if others are printable
    printable = 0
    res = []
    for j in range(5):
        pj = xor(current_c[j][:len(crib)], key_part)
        res.append(pj)
        if all(32 <= c <= 126 for c in pj):
            printable += 1
    
    if printable >= 3:
        print(f"Combination {bin(i)}:")
        for j, pj in enumerate(res):
            print(f"  p{j}: {pj}")
