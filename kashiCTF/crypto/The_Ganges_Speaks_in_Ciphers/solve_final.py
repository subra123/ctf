...
# Use p1 = "Hello! To the ghats of Varanasi" but we need to know the swapcase state
# c1 = swapcase(p1 ^ K)
# c4 = (p4 ^ K)
# So c1 ^ c4 = swapcase(p1 ^ K) ^ (p4 ^ K)
# If p1 ^ K consists of letters, then c1 ^ c4 = p1 ^ p4 ^ 0x20

# Let's try to find K such that p1 = "Hello! To the ghats of Varanasi"
# p1 ^ K = swapcase(c1)
# K = swapcase(c1) ^ p1

target_p1 = b"Hello! To the ghats of Varanasi"
K = xor(c_bytes[1].swapcase()[:len(target_p1)], target_p1)

for i in range(5):
    # Try both ways (identity and swapcase)
    p_i = xor(c_bytes[i][:len(K)], K)
    p_i_swap = xor(c_bytes[i].swapcase()[:len(K)], K)
    print(f"p{i}: {p_i} OR {p_i_swap}")
